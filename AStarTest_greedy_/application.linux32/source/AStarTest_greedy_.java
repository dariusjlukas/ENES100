...
public class AStarTest_greedy_ extends PApplet {
...
public void mousePressed(){
 occupancy[PApplet.parseInt((mouseX - cWidth/4)/cWidth)][PApplet.parseInt((mouseY - cHeight/4)/cHeight)] = true; 
}

////Custom Functions////

public void shortestPath() {  //Calculate the shortest path
  boolean done = false;
  ...
  currentNode[0] = PApplet.parseInt((roverX - cWidth/2)/cWidth);  //Set the initial node column
  currentNode[1] = PApplet.parseInt((roverY - cHeight/2)/cHeight); //Set the initial node row
  ...
  while (done == false) {
    ...
    if(PApplet.parseInt(que[0][0]) == donePile[donePileCount][2] && que[0][1] == donePile[donePileCount][3]){  //Make sure the node on the top of the que was not the previous node 
       currentNode[0] = PApplet.parseInt(que[1][0]);
      currentNode[1] = PApplet.parseInt(que[1][1]);
    }
    else{
      currentNode[0] = PApplet.parseInt(que[0][0]);
      currentNode[1] = PApplet.parseInt(que[0][1]);
    }
...
public float cost(float c, float r) {    //Calculate the cost of the given point

  return(sqrt((targetX-c*(cWidth)+cWidth/2)*(targetX-c*(cWidth)+cWidth/2) + (targetY-r*(cHeight)+cHeight/2)*(targetY-r*(cHeight)+cHeight/2)));
}

public void reconstructPath(int [][] donePile) {    //Reconstruct the path used to get to the target
  ...
  shortestPath[lastElementPos + 1][0] = PApplet.parseInt((targetX - cWidth/2)/cWidth);
  shortestPath[lastElementPos + 1][1] = PApplet.parseInt((targetY - cHeight/2)/cHeight);
...
public boolean inDonePile(int x, int y, int [][] donePile, int donePileCount){
 ...
 //}
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--stop-color=#cccccc", "AStarTest_greedy_" };
    if (passedArgs != null) {
